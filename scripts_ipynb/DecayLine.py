    ...
    def set_data(self, x=None, y=None):
        if x is None or y is None:
            ...
            if len(self.points)>1:
                ...
                if self.lc is None:
                    self.lc = LineCollection(self.segments, colors=self.get_colors(), linewidths=1.0, zorder=self._zorder, label="" if self._label is None else self._label)
                ...
                if self.lc is None:
                    self.lc = LineCollection([], linewidths=1.0, colors=(self.rgb_color[0], self.rgb_color[1], self.rgb_color[2], 1.0), zorder=self._zorder, label="" if self._label is None else self._label)
    ...
    def get_colors(self):
        ...
        if  n < 2:
            ...
            if isinstance(self.rgb_color, np.ndarray):
                colour = np.concatenate([self.rgb_color, [1.0,]])
